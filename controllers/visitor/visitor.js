...
const { visitor, company, driver } = require("../../models/zindex");
...
exports.createVisitor = asyncHandler(async (req, res) => {
  ...
  // Extract mobile number from the dynamic field (case-sensitive)
  const mobile = formData["Mobile No"] || formData["mobile"] || "";

  if (!mobile) {
    return response.badRequest("Mobile number is required.", res);
  }
...
exports.getDriverRequest = asyncHandler(async (req, res) => {
  const driverId = req.user._id;

  const visitors = await visitor.find({ driverId: driverId }).populate('companyId');
  
  return response.success("Driver Request Fetched", visitors, res);
});


exports.getHotelDriverRequest = asyncHandler(async (req, res) => {
  const companyId = req.user._id;

  const visitors = await visitor.find({ companyId: companyId }).populate('driverId');
  
  return response.success("Hotel Driver Request Fetched", visitors, res);
});

// controllers/visitor.js
exports.acceptHotelDriverRequest = asyncHandler(async (req, res) => {
  ...
  return response.success("Driver request accepted", updatedVisitor, res);
});
...
const existingDriver = await driver.findOne({ mobile });
  if (!existingDriver) {
    return response.notFound("Driver not found with this mobile", res);
  }
